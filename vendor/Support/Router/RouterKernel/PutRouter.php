...
{   
    ...
    private static  $METHOD_POST = 'PUT';
    public static function put(string $path, $handler): void
    {
        $path = self::cleanRequestPath($path);
        self::$putHandler = array_merge(
            self::addHandler(self::$putHandler, self::$METHOD_POST, $path, $handler),
            self::$putHandler
        );
    }
    public static function putExec($requestUri, $requestPath)
    {
       $valid = self::validateRoute(self::$putHandler,self::$METHOD_POST,$requestUri, $requestPath);
       $callback = @self::$putHandler[self::$METHOD_POST.$requestPath]['handler'];
